    ...
    return struct {
        ...
        fn on_start_view(self: *Self, message: *const Message.StartView) void {
            ...
            //  Cluster is far ahead, replica's WAL no longer intersects primary's WAL.
                ...
                // Cluster is a bit ahead. The replica might still be able to repair WAL.
                ...
                self.syncing == .awaiting_checkpoint))
            {
                ...
                switch (self.syncing) {
                    ...
                    => {
                        log.debug(
                            \\{}: on_start_view view={} sync waiting (op_prepare_max={} message.op={})
                        , .{
                            self.replica,
                            self.log_view,
                            self.op_prepare_max(),
                    ...
                    .updating_superblock => {
                        log.debug(
                            \\{}: on_start_view view={} sync in progress (op_prepare_max={} message.op={})
                        , .{
                            self.replica,
                            self.log_view,
                            self.op_prepare_max(),
                ...
                log.debug(
                    \\{}: on_start_view view={} sync started (op_prepare_max={} message.op={})
                , .{
                    self.replica,
                    self.log_view,
                    self.op_prepare_max(),
            ...
            const op_prepare_max_effective = if (self.syncing == .updating_superblock)
                vsr.Checkpoint.prepare_max_for_checkpoint(
            ...
            for (view_headers.slice) |*header| {
                ...
                if (header.op <= op_prepare_max_effective) {
